...
$condition = str_replace(' AND ' . $is_active_cond, '', $condition);
$params['sort_by'] = 'ab__dotd_expired';
$params['sort_order'] = 'descasc';
$sortings['ab__dotd_expired'] = array("($is_active_cond)", db_quote('ABS((to_date/-1) + ?i)', TIME));
}
...
$promotion['ab__dotd_active'] = (empty($promotion['from_date']) || $promotion['from_date'] <= TIME) && (empty($promotion['to_date']) || $promotion['to_date'] >= TIME);
}
}
...
$where .= db_quote(' ?p ab__products_categories.category_id ?p (?n)', $and_or, $operators[$conditions['set_value']][$condition['operator']], $condition['value']);
