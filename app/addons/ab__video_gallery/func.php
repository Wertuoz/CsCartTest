...
'?:ab__video_gallery.width',
'?:ab__video_gallery.height',
...
db_replace_into('ab__video_gallery_settings', array(
...
$settings = db_get_hash_single_array('SELECT var, value FROM ?:ab__video_gallery_settings WHERE product_id = ?i', array('var', 'value'), $product_id);
return $settings;
}
...
$old_video_ids = db_get_fields('SELECT video_id FROM ?:ab__video_gallery WHERE product_id = ?i', $product_id);
$new_video_ids = fn_array_column($product_data['ab__vg_videos'], 'video_id');
$deleted_ids = array_diff($old_video_ids, $new_video_ids);
foreach ($deleted_ids as $video_id) {
fn_ab__vg_delete_video($video_id);
}
$new_video_ids = array();
...
if ($video_id) {
$new_video_ids[$key] = $video_id;
}
}
fn_attach_image_pairs('video_icon', 'ab__vg_video', 0, $lang_code, $new_video_ids);
}
}
