...
class CashRegister implements ICashRegister
{
    ...
    const TEST_API_URL = 'https://testonline.atol.ru/possystem/v3/';
    ...
    public function __construct(
        ...
        $callback_url, Http $http_client, RequestLogger $request_logger, $mode = 'live'
    ) {
        $this->inn = (string) $inn;
    ...
    {
        if ($this->mode == 'test') {
            $url = sprintf('%s/', self::TEST_API_URL);
        } else {
            $url = sprintf('%s/', self::API_URL);
        }
        return $url;
    }
