...
use Tygh\Enum\Addons\Pickup\MapLanguages;
    ...
    if (!empty($cart['shippings_extra']['data'])) {
        ...
        if (!empty($select_store)) {
            ...
            foreach ($select_store as $g_key => $g) {
                foreach ($g as $s_id => $s) {
                    if (!empty($cart['shippings_extra']['data'][$g_key][$s_id]) && !empty($cart['shippings_extra']['data'][$g_key][$s_id]['stores'])) {
 ...
 * @return string
 */
function fn_rus_pickup_get_map_language($lang_code)
{
    $lang_array = MapLanguages::getLanguages();
    $key = array_search($lang_code, $lang_array);

    if ($key !== false) {
        $locale = Languages::getLocaleByLanguageCode($lang_array[$key]);
    } else {
        $locale = MapLanguages::EN;
    }
