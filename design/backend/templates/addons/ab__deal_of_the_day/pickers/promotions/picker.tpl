...
{$extra_class = ""}
{/if}
{include file="addons/ab__deal_of_the_day/pickers/promotions/js.tpl" promotion_id=$p_id holder=$data_id input_name=$input_name hide_link=$hide_link hide_delete_button=$hide_delete_button hide_input=true first_item=$smarty.foreach.items.first position_field=$positions position=$smarty.foreach.items.iteration+$start_pos}
</div>
{$smarty.capture.add_buttons nofilter}
</div>
{/foreach}
...
{/if}
{if $multiple}
{include file="addons/ab__deal_of_the_day/pickers/promotions/js.tpl" promotion_id="`$ldelim`promotion_id`$rdelim`" holder=$data_id input_name=$input_name clone=true hide_link=$hide_link hide_delete_button=$hide_delete_button hide_input=true position_field=$positions position="0"}
{/if}
{if $multiple}
</tbody>
