{capture name="mainbox_title"}{__("abt__ut_buy_together.manage")}{/capture}
...
<li>{include file="common/popupbox.tpl" id="abt_group_bt_`$chain.chain_id`" act="edit" link_text=__("edit") href="buy_together.update?chain_id=`$chain.chain_id`" no_icon_link=true link_class="abt__ut_bt-edit_form"}</li>
<li>{btn type="list" text=__("delete") class="cm-confirm" href="abt__ut_buy_together.delete?chain_id=`$chain.chain_id`" method="POST"}</li>
{/capture}
...
{capture name="tools_list"}
{if $chains}
<li>{btn type="delete_selected" dispatch="dispatch[abt__ut_buy_together.m_delete]" form="manage_chains_form"}</li>
{/if}
{/capture}
{dropdown content=$smarty.capture.tools_list}
{/capture}
...
{include file="addons/abt__unitheme/views/abt__ut_buy_together/components/chains_search_form.tpl" dispatch="abt__ut_buy_together.manage"}
{/capture}
