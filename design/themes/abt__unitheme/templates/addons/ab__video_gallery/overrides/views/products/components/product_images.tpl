...
<div class="ty-product-img cm-preview-wrapper" {if $settings.Appearance.thumbnails_gallery == "N"}style="display:inline-block"{/if}>
    ...
    {if $ab__vg_videos}
        {if $addons.ab__video_gallery.on_thumbnail_click == 'image_replace'}
            ...
            {foreach $ab__vg_videos as $video}
                <div id="det_img_link_{$preview_id}_{$video.video_id}"
                     class="ab__vg_loading ab__vg-image_gallery_video cm-image-previewer{if $ab__vg_settings.replace_image != 'Y' || $video@iteration > 1} hidden{/if}"
                     data-width="{$ab__video_width|default:$video.width}"
                     data-height="{$ab__video_height|default:$video.height}"
                     data-src="{include file="addons/ab__video_gallery/components/build_url.tpl" youtube_id=$video.youtube_id}"
        ...
        {foreach $ab__vg_videos as $video}
            ...
            <div id="ab__vg_video_{$video.video_id}" class="cm-popup-box hidden" title="{$video.title}">
                <div style="{if $video.width}width:{$video.width}px;{/if}{if $video.height}height:{$video.height}px{/if}"
                     {if $video.width}data-width="{$video.width}"{/if} {if $video.height}data-height="{$video.height}"{/if} class="ab__vg_loading" data-src="{include file="addons/ab__video_gallery/components/build_url.tpl" youtube_id=$video.youtube_id}" data-frameborder="0" data-allowfullscreen="1">
                    {include file="addons/ab__video_gallery/components/thumbnail.tpl" video=$video width=$ab__video_width|default:$video.width height=$ab__video_height|default:$video.height}
                </div>
            </div>
        {/foreach}
    {/if}
    <p class="ty-center"><small style="color:#999;">{__('hover_to_enlarge')}</small></p>
</div>
                ...
                {if $ab__vg_videos && $addons.ab__video_gallery.position == 'pre'}
                    {foreach $ab__vg_videos as $video}
                        <a data-ca-gallery-large-id="det_img_link_{$preview_id}_{$video.video_id}"
                           class="ab__vg-image_gallery_item ty-product-thumbnails__item cm-thumbnails-mini{if $ab__vg_settings.replace_image == 'Y' && $video@iteration == 1} active{/if}">
                            <img src="{$images_dir}/addons/ab__video_gallery/youtube_logo.png" width="{$th_size}" height="{$th_size}" alt="{$video.title|strip_tags}"/>
                        </a>
                    {/foreach}
                {/if}
