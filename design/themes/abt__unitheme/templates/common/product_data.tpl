...
{capture name="discount_label_`$obj_prefix``$obj_id`"}
    {if $show_discount_label && ($product.discount_prc || $product.list_discount_prc) && $show_price_values}
        ...
        {if $product.list_discount_prc > 0}
            {assign var="prc" value="list_discount_prc"}
        {/if}
        <span class="ty-discount-label {if $product.$prc <= 25}color1{/if} {if $product.$prc > 25 && $product.$prc <= 50}color2{/if} {if $product.$prc > 50}color3{/if} cm-reload-{$obj_prefix}{$obj_id}" id="discount_label_update_{$obj_prefix}{$obj_id}">
        <span class="ty-discount-label__item" id="line_prc_discount_value_{$obj_prefix}{$obj_id}"><span class="ty-discount-label__value" id="prc_discount_value_label_{$obj_prefix}{$obj_id}">{__("save_discount")}<br><em>{if $product.discount}{$product.discount_prc}{else}{$product.list_discount_prc}{/if}%</em></span></span>
                    ...
                    {if ($product_amount > 0 && $product_amount >= $product.min_qty) && $settings.General.inventory_tracking == "Y" || $details_page}
                        ...
                        {elseif $settings.General.inventory_tracking == "Y" && $settings.General.allow_negative_amount != "Y"}
                            <div class="ty-control-group product-list-field">
                                ...
                                {/if}
                                <span class="ty-qty-out-of-stock ty-control-group__item"><i class="ty-icon-ok"></i> {$out_of_stock_text}</span>
                            </div>
                        {/if}
                    {/if}
                {/if}
                ...
                {elseif $details_page
                    ...
                    <div class="ty-control-group product-list-field">
                        {if $show_amount_label}
                            <label class="ty-control-group__label">{__("availability")}:</label>
                        {/if}
                        <span class="ty-qty-out-of-stock ty-control-group__item" id="out_of_stock_info_{$obj_prefix}{$obj_id}"><i class="ty-icon-ok"></i> {$out_of_stock_text}</span>
                    </div>
                {/if}
            {/if}
        {/if}
