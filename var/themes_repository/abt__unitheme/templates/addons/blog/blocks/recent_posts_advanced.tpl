...
{if $items}
    ...
    <div class="ty-mb-l">
        ...
        {if $settings.abt__unitheme.blog_page_id.value|intval > 0}
            <div class="ty-mtb-xs ty-left">
                {include file="buttons/button.tpl" but_href="pages.view?page_id=`$settings.abt__unitheme.blog_page_id.value|intval`" but_text=__("view_all") but_role="text" but_meta="blog-ty-text-links__button"}
            </div>
        {/if}
    </div>
{/if}