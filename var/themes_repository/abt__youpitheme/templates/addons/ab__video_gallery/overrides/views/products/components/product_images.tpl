{assign var="th_size" value=$thumbnails_size|default:40}
...
<div class="ty-product-img {if ($product.image_pairs|count + $ab__vg_videos|count) > 6}two-col{elseif $product.image_pairs || $ab__vg_videos}one-col{/if} cm-preview-wrapper">
    ...
    <div class="ypi-text-image-zoom ty-center"><small><i class="material-icons">&#xE8FF;</i>{__('abt__yt.hover_to_enlarge')}</small></div>
</div>
...
{if $product.image_pairs || $ab__vg_videos}
    {if $settings.Appearance.thumbnails_gallery == "Y"}
        ...
        {/strip}
    {else}
        <div class="ty-product-thumbnails ty-center cm-image-gallery" id="images_preview_{$preview_id}" style="{if ($product.image_pairs|count + $ab__vg_videos|count) > 6}width:{($th_size * 2) + 24}px;{else}width:{$th_size + 12}px;{/if}">
            {strip}
