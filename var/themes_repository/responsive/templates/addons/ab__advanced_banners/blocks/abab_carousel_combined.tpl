{* block-description:abab_carousel_combined *}
{if $items}
{** отслеживаем https **}
...
{$image_data=$banner.main_pair|fn_image_to_display:$image_width:$image_height}
<img src="{$image_data.image_path}" alt="{$banner.main_pair.icon.alt}" title="{$banner.main_pair.icon.alt}" style="max-height:{$block.properties.min_height};width:auto">
</div>
{/if}
