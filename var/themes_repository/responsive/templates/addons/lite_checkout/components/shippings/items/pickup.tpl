...
>
    <input type="radio" 
            class="ty-one-store__radio-{$group_key} cm-map-select-store"
    ...
    <div class="ty-sdek-store__label ty-one-store__label">
        <p class="ty-one-store__name">
            ...
            {if $store.pickup_surcharge && $store.pickup_surcharge > 0}
            <span class="ty-one-store__name-rate">
                ({include file="common/price.tpl" value=$store.pickup_surcharge})
            </span>
            {/if}
        </p>
